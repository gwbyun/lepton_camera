...
def ktof(val):
    return np.round(((1.8 * ktoc(val) + 32.0)), 2)
...
def display_temperatureC(img, val_k, loc, color):
    ...
    x, y = loc
    cv2.line(img, (x - 2, y), (x + 2, y), color, 2)
    cv2.line(img, (x, y - 2), (x, y + 2), color, 2)
...
def blocks(data):
    ...
    for i in range(0,120,10):
        for j in range(0,160,10):
            block = data[i:i+10, j:j+10]
            blocks.append(block)
    new_matrix = np.zeros((12,16))
    for i in range(12):
        for j in range(16):
            block = blocks[i*16+j]
            block_mean = np.mean(block)
            new_matrix[i, j] = block_mean
...
def getFrame():
    ...
    tem_data = ktoc(np.array(data))
    print(tem_data)
    tem_data_average = blocks(tem_data)
    #raw_data = data
    #print(tem_data_average.shape)
    if data is None:
        print('No Data')
    pixel2csv(tem_data_average,'file.csv')
    ...
    img = cv2.applyColorMap(raw_to_8bit(data), cv2.COLORMAP_HOT)
    ...
    display_temperatureC(img, minVal, minLoc,(255,0,0))
    

    return img, tem_data_average    
...
class MyThread(threading.Thread):
    def __init__(self):
        ...
        #self.fig = plt.figure()  # fig 객체 초기화
    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            sns.heatmap(data,cmap='hot',annot=True ,cbar = False,fmt=".1f")
            plt.axis('off')  # 축 제거
            
            # 그림을 이미지로 변환
            fig = plt.gcf()
            fig.tight_layout()  # 주변 여백 최소화
            #fig.set_size_inches(640, 480)  # 원하는 크기로 조정
            
            fig.canvas.draw()
            image = np.array(fig.canvas.renderer.buffer_rgba())
            image_bgr = cv2.cvtColor(image, cv2.COLOR_RGBA2BGR)
            # OpenCV 창에 이미지 표시
            resized_image = cv2.resize(image_bgr, (640, 480))  # 원하는 크기로 조정

            cv2.imshow('Heatmap', resized_image)
            cv2.waitKey(1)
                
            cv2.namedWindow('view', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('viwe', 640, 480)
...
def pixel2csv(data,file_path):
    
    
    elapsed_time = datetime.now() - start_time
    column_name = str(elapsed_time)
    
    #file_path = 'file.csv'
    #data = np.array([[1,2,3,4,],[2,3,4,5,],[5,6,7,8,],[7,7,7,7,]])

    pixel_values = data.flatten().tolist()
    time_pixel = [column_name]+pixel_values
    #print(time_pixel)
    df_new = pd.DataFrame([time_pixel])

    if os.path.isfile(file_path):
        df_old = pd.read_csv(file_path)
            
        df_old.loc[len(df_old)] = time_pixel
