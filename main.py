...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 26 18:09:35 2023
...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stopped:
            ...
            # heatmap 이미지를 생성합니다.
            heatmap = cv2.applyColorMap(normalized_data, cv2.COLORMAP_HOT)
            cv2.namedWindow('Heatmap', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('Heatmap', 640, 480)
            print("update")
            ...
            cv2.resizeWindow("Graph", 800, 600)
            
            cv2.imshow("Graph", image_bgr)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
            '''
